$(function() {
	...
	/*
	function hide() {
		var width = parseInt($menubox.css("width"));
		$menubox.css({
			left: -width
		});
		setTimeout(function() {
	...
	*/
	$menu.click(function(e) {
		...
		var left = parseInt($menubox.css("left")),
			menuMarginTop = parseInt($menubox.css("margin-top")),
			menuHeight = $menubox.height() + menuMarginTop,
		...
		if (left === 0) {
			...
			if (menuHeight>documentHeight - $copyright.height()) {
				$footer.css("top", menuHeight - $links.height());
			} else {
				//$menubox.height(documentHeight- menuMarginTop - $copyright.height());
			}
		}
		e.stopPropagation();
		*/
	...
	var originalDocumentHeight = $document.height();
	$window.load(function() {
		originalDocumentHeight = $document.height();
		console.log(originalDocumentHeight);
	});
	

	$menubox.click(function(e) {
		
		//console.log($menubox.height() + parseInt($menubox.css("margin-top")));




		//$footer.css("top",$(".menubox").height() + parseInt($menubox.css("margin-top")) - $links.height());



		/*
		var menuMarginTop = parseInt($menubox.css("margin-top")),
			menuHeight = $menubox.height() + menuMarginTop,
			documentHeight;
		$menubox.hide();
		...
		if (menuHeight>documentHeight - $copyright.height()) {
			...
			console.log(1)
			$menubox.height($document.height() - menuMarginTop - $copyright.height());
		}*/
		e.stopPropagation();
	});
	...
	(function() {
		...
		function getdata(data) {
			...
			for(var i=0 ;i<22;i++) {
				$menubox.append("<div style=\"height:50px;background-color:yellow;\" class=\"add\">add</div>");
				$menubox.append("<div style=\"height:50px;background-color:yellowgreen;\" class=\"remove\">remove</div>");
			}


			$menubox.on("click",".add",function() {
				$menubox.append("<div style=\"height:50px;background-color:yellow;\" class=\"add\">add</div>");
			});

			$menubox.on("click",".remove",function() {
				$(this).remove();
			});
